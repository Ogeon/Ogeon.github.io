...
Rustful provides a set of common presets which makes it as easy throwing a
...
Not too complicated, right? I'll still go through them, just in case:
  ...
  2. `handlers: router`: The `handler` field contains the request handlers.
  ...
  logger is included in the `Context` type, among other.
...
what that it should be possible to just throw a function into a router and it
...
This is only the beginning, and there are still a things to do (especially
